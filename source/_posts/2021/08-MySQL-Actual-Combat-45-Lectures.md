...
<img src="https://static001.geekbang.org/resource/image/68/ed/68d08d277a6f7926a41cc5541d3dfced.png" style="width:400px"/>

图中虚线框里是同一行数据的 `4` 个版本，当前最新版本是 `V4`，`k` 的值是 `22`，它是被 `transaction id` 为 `25` 的事务更新的，因此它的 `row trx_id` 也是 `25`。

图中的三个虚线箭头其实就是 `undo log`；而 `V1`、`V2`、`V3` 并不是物理上真实存在的，而是每次需要的时候根据当前版本和 `undo log` 计算出来的。比如，需要 `V2` 的时候，就是通过 `V4` 依次执行 `U3`、`U2` 算出来。
...
<img src="https://static001.geekbang.org/resource/image/88/5e/882114aaf55861832b4270d44507695e.png" style="width:400px"/>
