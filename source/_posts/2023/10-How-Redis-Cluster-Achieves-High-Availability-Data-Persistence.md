...
从 `RDB` 快照中恢复数据通常比从 `AOF` 日志中恢复要更快，因为前者直接按原样存储键值对，而后者是所有变更操作的时间线，需要从日志起始重放。然而，快照的执行频率需要权衡：如果执行频率过高，将会产生较大的资源开销；但是如果执行频率太低，那么当 `Redis` 发生故障时，尚未保存到快照的变更数据将会丢失。那有没有办法结合两个方案的优点？理想情况下，我们希望 `RDB` 快照提供快速数据恢复能力，同时最大限度地减少数据丢失和资源占用。

`Redis 4.0` 版本提出了 `AOF` 日志和 `RDB` 快照结合的方式。简单来说，`RDB` 快照以一定的频率执行，在两个快照之间，`AOF` 日志用于持久化所有增量数据变更。在这种场景下，执行快照的频率可以较低，避免主进程频繁 `fork` 操作带来的资源开销。另外，`AOF` 日志只是用来记录两个快照之间的变化，而不是记录从始到终的整个历史，日志文件不会过大，压缩/重写所占用的资源也相应减少。
...
`Redis` 支持创建多个副本，每个分区可以有一个写副本（`leader`）和多个读副本（`follower`）。当一个新的从节点实例加入集群时，它会首先从主节点那里加载最新的 `RDB` 快照文件，然后开始消费复制缓冲区增量数据保持与主节点同步。
...
`Redis` 集群也使用哨兵节点来保证高可用性。`Sentinel` 节点是特殊的 `Redis` 节点（是相对于存储键值数据的主节点和从节点，又名数据节点而言），`Sentinel` 节点有 3 个作用：

- **监控**：监听数据节点的心跳，判断哪些节点健康，哪些不可用；
- **领导选举**：当旧的 `leader` 节点挂掉或者没有响应时，负责选出一个新的 `leader` 节点；
- **通知**：通知从节点与主节点保持同步。
...
如果它是一个从节点，不管从节点是否真的活着——也许这个节点只是暂时不可用，很快就会回来——它仍然会被哨兵节点标记为死亡，并停止接收服务流量。

如果是主节点，我们在设计哨兵节点逻辑的时候就要考虑到可能的误报。主节点可能完全活跃并正常提供服务，但只是它与哨兵的连接遇到问题——也许数据中心的网络出现了一些故障。如果触发了领导者故障转移，随后的领导者选举和通知将产生大量的计算和网络资源占用，进一步给网络带来压力，并可能触发雪崩故障。
...
配置属性 `slave-priority` 可用于为每个从节点设置用户定义的优先级。例如，假设我们有 `2` 个具有不同内存空间的从节点实例，我们可以为具有更多内存的实例设置高优先级，在领导者选举中，哨兵节点将首先给具有最高用户定义优先级的节点打分——因此拥有更多内存的节点将成为新的主节点。

在两个从节点的 `slave-priority` 配置相同的情况下，哨兵节点将使用第二条规则来确定新的主节点，原因是在这种情况下，新领导者将拥有最新的数据。众所周知，主从复制可能由于网络传输延迟而存在复制滞后性，在复制过程中，主节点使用 `master_repl_offset` 记录 `repl_backlog_buffer` 中最近一次写入的日志位置，从节点使用 `slave_repl_offset` 记录其最近申请的写入的日志位置。
...
在分片集群中数据分布在所有实例中。`Redis` 使用哈希槽将键值记录映射到实例。在当前的 `Redis` 实现中，最多有 `16384` 个哈希槽。每个键值记录都散列到其中一个槽中。具体来说，

- `CRC16` 哈希用于使用键值记录的 `Key` 计算 `16` 位哈希值；
- 然后通过计算后的 `16` 位哈希值和 `16384` 的模来计算 `[0, 16383]` 范围内的槽号。

简单来说，有两层映射：

- 一个键值记录首先映射到一个哈希槽，这种映射是确定性的，由计算完成。
- 然后将哈希槽映射到特定的 `Redis` 实例。此映射关系存储在所有 `Redis` 实例上，并通过 `gooisp` 协议在所有实例之间保持同步。当我们向集群添加/删除实例时，或者当 `Redis` 决定将哈希槽重新分配给实例以平衡每个实例上的负载时，这个映射关系是可以动态调整的，并且可能会发生变化。
...
有关 `Redis` 分片的更多详细信息，请查看我在[深入研究 Redis 集群：分片算法和体系结构](https://medium.com/@bb8s/deep-dive-into-redis-cluster-sharding-algorithms-and-architecture-a093a92e97af)上的文章。
