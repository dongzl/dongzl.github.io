...
另一方面，**负载均衡负责在微服务的多个实例之间分发传入请求**，以提升服务可用性、性能和可扩展性。它有助于*在多个实例之间均匀分配工作负载*，并确保每个服务实例都得到充分利用。
...
2. **建立安全机制**：`API` 网关可以为所有微服务实施安全策略，例如身份认证和授权；
...
这就是**微服务中 API 网关和负载均衡之间的差异**。正如我前面所说，`API` 网关提供与 `API` 管理相关的**更高层级的能力**，例如安全性、版本控制、单一流量入口；而负载均衡提供与跨微服务的多个实例的流量分发相关的**较低层级的能力**。
