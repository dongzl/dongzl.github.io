...
- 从 `ClickHouse` 查看 `MySQL` 数据。可以使用原生 `ClickHouse` `SQL` 语法通过 `ClickHouse` 查询 `MySQL` 数据。这对于探索数据以及加入 MySQL 是记录系统的数据很有用。
- 将数据从 MySQL 永久移动到 ClickHouse。 ClickHouse 成为该数据的记录系统。这会卸载 MySQL 并为分析提供更好的结果。
- 在 ClickHouse 镜像 MySQL 数据。将数据快照放入 ClickHouse 并使用复制保持更新。这允许用户在不增加交易处理负担的情况下询问有关交易数据的复杂问题。
...
在 `ClickHouse` 方面，我们通常会选择使用 `MergeTree` 表引擎或其变体之一，例如 `ReplicatedMergeTree`。`MergeTree` 是 `ClickHouse` 上大数据的首选引擎，以下三个重要功能将帮助我们充分利用 `ClickHouse`。

@@@@@@@@@@@@@@@@@@@@图片@@@@@@@@@@@@@@@@@@@@

1. **分区**——`MergeTree` 使用分区键将表分成多个部分，访问日志内容和其他大数据内容往往是按时间排序的，因此通常按天、按周或按月来划分数据；为获得最佳性能，建议设置的分区数少于 `1000`；
2. **排序**——`MergeTree` 可以对数据进行排序并在行上构建索引以匹配我们所选择的排序，重要的是确定一种排序方式，以便在扫描数据时为我们提供大量“运行”；例如我们可以选择按租户排序，然后再按时间排序，这意味着对租户数据的查询不需要跳来跳去查找与该租户相关的数据行；
3. **压缩和编解码器**——`ClickHouse` 默认使用 `LZ4` 压缩，但也提供 `ZSTD` 压缩和编解码器；编解码器在将列数据转为压缩之前减少列数据内容。
...
`ClickHouse` `MySQL` 数据库引擎在这种情况下也非常有用，它使 `ClickHouse` 能够从 `MySQL` 的远程事务表中“查看”和选择数据。我们的 `ClickHouse` 查询可以连接本地表的交易数据，其自然归宿是 `MySQL`。同时，`MySQL` 高效且安全地处理事务性更改。

将表迁移到 `ClickHouse` 通常按以下方式进行。我们将使用上面显示的访问日志示例。

1. 为 ClickHouse 上的访问日志数据创建匹配模式；
2. 使用以下任何工具将数据从 MySQL 转储/加载到 ClickHouse：
   - [Mydumper](https://github.com/mydumper/mydumper) - 处理 mysqldump 和 CSV 格式的并行转储/加载实用程序；
   - [MySQL Shell](https://dev.mysql.com/doc/mysql-shell/8.0/en/mysql-shell-features.html) - 用于管理可以导入和导出表的 MySQL 的通用实用程序；
   - 在 MySQL 数据库引擎表上使用 SELECT 复制数据；
   - 本机数据库命令 – 使用 MySQL SELECT OUTFILE 将数据转储到 CSV 并使用 ClickHouse INSERT SELECT FROM file() 读回。 ClickHouse 甚至可以读取 mysqldump 格式。
3. 使用合适的查询检查性能；对模式进行调整并在必要时重新加载；
4. 适配前端和访问日志摄取到 ClickHouse；
5. 测试时并行运行两个系统；
6. 从仅 `MySQL` 切换到 `MySQL` `ClickHouse` 扩展；

迁移可能只需要几天时间，但在大型系统中更常见的是需要几周到几个月的时间。这有助于确保一切都经过适当测试，并顺利推出。

### 在 ClickHouse 中存储 MySQL 数据镜像

另一种扩展 MySQL 的方法是镜像 ClickHouse 中的数据，并使用复制使数据保持最新。镜像允许用户在不 (a) 更改 MySQL 及其应用程序或 (b) 影响生产系统性能的情况下，对交易数据运行复杂的分析查询。

以下是镜像设置的工作部分。

ClickHouse 有一种内置的方式来处理镜像：实验性的[MaterializedMySQL 数据库引擎](https://clickhouse.com/docs/en/engines/database-engines/materialized-mysql)，它直接从 MySQL 主数据库读取二进制日志记录，并将数据传播到 ClickHouse 表中。该方法很简单，但尚未推荐用于生产用途。它最终可能对 1 对 1 镜像情况很重要，但在广泛使用之前需要额外的工作。
...
外部化方法有许多优点。它们包括使用当前的 ClickHouse 版本，利用快速转储/加载程序（如 mydumper 或使用 MySQL 数据库引擎的直接 SELECT），支持镜像到复制表，以及添加新表或重置旧表的简单过程。最后，它可以扩展到复制到单个 ClickHouse 集群的多个上游 MySQL 系统。

@@@@@@@@@@@@@@@@@@@@图片@@@@@@@@@@@@@@@@@@@@

由于[ReplacingMergeTree](https://clickhouse.com/docs/en/engines/table-engines/mergetree-family/replacingmergetree)表的独特功能，ClickHouse 可以从 MySQL 镜像数据。它有一种处理插入、更新和删除的有效方法，非常适合用于复制数据。如前所述，ClickHouse 无法轻松更新单个行，但它插入数据的速度非常快，并且具有在后台合并行的高效过程。 ReplicatingMergeTree 以这些功能为基础，以“ClickHouse 方式”处理数据更改。

复制的表行使用版本和符号列来表示更改行的版本以及更改是插入还是删除。 ReplacingMergeTree 只会保留一行的最后一个版本，实际上可能已将其删除。符号列让我们应用另一个 ClickHouse 技巧来使那些已删除的行不可访问。这称为[行策略](https://clickhouse.com/docs/en/sql-reference/statements/create/row-policy)。使用行策略，我们可以使符号列为负的任何行消失。

下面是 ReplacingMergeTree 的一个示例，它结合了版本和符号列的效果来处理可变数据。

@@@@@@@@@@@@@@@@@@@@图片@@@@@@@@@@@@@@@@@@@@

将数据镜像到 ClickHouse 可能看起来比迁移更复杂，但实际上相对简单，因为不需要更改 MySQL 架构或应用程序，并且 ClickHouse 架构生成遵循千篇一律的模式。实施过程包括以下步骤。

1. 在 ClickHouse 中为复制表创建模式。
2. 配置并启动从 MySQL 到 ClickHouse 的复制。
3. 使用与迁移相同的工具将数据从 MySQL 转储/加载到 ClickHouse。

此时，用户可以自由地开始在 ClickHouse 上运行分析或构建其他应用程序，同时不断从 MySQL 复制更改。

## 工具改进正在进行中！

MySQL 到 ClickHouse 的迁移是 Altinity 和整个 ClickHouse 社区都在积极开发的一个领域。改进分为三大类。

转储/加载实用程序——Altinity 正在开发一种新的实用程序来移动数据，减少模式创建和数据传输到一个单一的。我们将在以后的博客文章中对此进行更多讨论。

复制——Altinity 正在赞助 ClickHouse 的接收器连接器，它可以自动执行高速复制，包括监控以及集成到 Altinity.Cloud 中。我们的目标同样是将复制设置减少到单个命令。

ReplacingMergeTree – 目前用户必须在表名中包含 FINAL 关键字以强制合并行更改。还需要添加行策略，让删除的行自动消失。正在进行拉取请求以添加 MergeTree 属性以在查询中自动添加 FINAL 以及使已删除的行在没有行策略的情况下消失。它们一起将使复制更新和删除的处理对用户完全透明。

我们也在仔细观察 MaterializedMySQL 的改进以及其他有效集成 ClickHouse 和 MySQL 的方法。您可以期待将来有更多关于这些和相关主题的博客文章。敬请关注！

## 总结和开始

ClickHouse 是对现有 MySQL 应用程序的强大补充。具有不可变数据的大型表、复杂的聚合管道以及有关 MySQL 事务的未解决问题都是明确的迹象，表明集成 ClickHouse 是为用户提供快速、经济高效的分析的下一步。

根据您的应用程序，使用复制将数据镜像到 ClickHouse 或什至将一些表迁移到 ClickHouse 可能是有意义的。 ClickHouse 已经与 MySQL 很好地集成，并且很快就会有更好的工具出现。不用说，Altinity 在该领域的所有贡献都是开源的，在 Apache 2.0 许可下发布。

最重要的一课是从 MySQL 和 ClickHouse 一起工作的角度来思考，而不是将其视为一个替代另一个。每个数据库都有独特而持久的优势。最好的应用程序将建立在这些之上，为用户提供比单独使用任何一个数据库更快、更灵活的功能。

开源数据库领域的知名专家 Percona 与 Altinity 合作，为 MySQL 应用程序提供强大的分析。如果您想了解更多关于 MySQL 与 ClickHouse 集成的信息，请随时联系我们或在我们的论坛上留言。
